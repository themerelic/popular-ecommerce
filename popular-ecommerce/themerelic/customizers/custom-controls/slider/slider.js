wp.customize.controlConstructor['relic-slider'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		jQuery( 'input[type=range]' ).on( 'mousedown', function() {
			...
			jQuery( this ).mousemove( function() {
				value = jQuery( this ).attr( 'value' );
				jQuery( this ).closest( 'label' ).find( '.relic_range_value .value' ).text( value );
			});
		});

		// Handle the reset button
		jQuery( '.relic-slider-reset' ).click( function() {
			thisInput    = jQuery( this ).closest( 'label' ).find( 'input' );
			inputDefault = thisInput.data( 'reset_value' );
			thisInput.val( inputDefault );
			thisInput.change();
			jQuery( this ).closest( 'label' ).find( '.relic_range_value .value' ).text( inputDefault );
		});
