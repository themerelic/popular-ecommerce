...
if( ! class_exists( 'Popular_ecommerce_Repeater_Control' ) ) {
    /**
     * Repeater control
     */
    class Popular_ecommerce_Repeater_Control extends WP_Customize_Control {
    	...
    	public function __construct( $manager, $id, $args = array() ) {
    		...
    		$this->row_label = array(
    			'type' => 'text',
    			'value' => esc_attr__( 'row', 'popular-ecommerce' ),
    			'field' => false,
    		);
    		...
    		if ( empty( $this->button_label ) ) {
    			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'popular-ecommerce' ), $this->row_label['value'] );
    		}
    			...
    			foreach ( $this->fields as $field ) {
    				if ( isset( $field['type'] ) ){
    					if( 'color' === $field['type'] ){ 
                            ...
                            wp_enqueue_style( 'font-awesome', get_template_directory_uri(). '/assets/font-awesome/css/font-awesome.css', array(), '4.7' );
    					}
    				}
    			}
    		}
            wp_enqueue_script( 'blossom-feminine-repeater', get_theme_file_uri() . '/themerelic/customizers/custom-controls//repeater/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
    		wp_enqueue_style( 'blossom-feminine-repeater', get_theme_file_uri() . '/themerelic/customizers/custom-controls//repeater/repeater.css', null );
    	}
    	...
    	protected function render_content() {
    		...
    		<?php if ( isset( $this->choices['limit'] ) ) : ?>
    			<p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'popular-ecommerce' ), esc_html( $this->choices['limit'] ) ); ?></p>
    		<?php endif; ?>
    		...
    		<script type="text/html" class="customize-control-repeater-content">
    			...
    			<li class="repeater-row minimized" data-row="{{{ index }}}">
    				...
    				<div class="repeater-row-content">
    					<# _.each( data, function( field, i ) { #>
    						...
    						<div class="repeater-field repeater-field-{{{ field.type }}}">
    							...
    							<# } else if ( 'color' === field.type ) { #>
    						        ...
    						        <label>
    						            ...
    						            <# if ( field.description ) { #>
    						                <span class="description customize-control-description">{{{ field.description }}}</span>
    						            <# } #>
    						            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'popular-ecommerce' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
    							...
    							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
    								...
    								<figure class="popular-ecommerce-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'popular-ecommerce' ); ?>" >
    									<# if ( field.default ) { #>
    										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
    										<img src="{{{ defaultImageURL }}}">
    									<# } else { #>
    										<?php esc_attr_e( 'No Image Selected', 'popular-ecommerce' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( 'Remove', 'popular-ecommerce' ); ?></button>
    									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'popular-ecommerce' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'popular-ecommerce' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_attr_e( 'Change Image', 'popular-ecommerce' ); ?>
    										<# } else { #>
    											<?php esc_attr_e( 'Add Image', 'popular-ecommerce' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    							...
    							<# } else if ( field.type === 'upload' ) { #>
    								...
    								<figure class="relic-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'popular-ecommerce' ); ?>" >
    									<# if ( field.default ) { #>
    									...
    									<# } else { #>
    										<?php esc_attr_e( 'No File Selected', 'popular-ecommerce' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
    									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'popular-ecommerce' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'popular-ecommerce' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_attr_e( 'Change File', 'popular-ecommerce' ); ?>
    										<# } else { #>
    											<?php esc_attr_e( 'Add File', 'popular-ecommerce' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
    					...
    					<# }); #>
    					<button type="button" class="button-link repeater-row-remove"><?php esc_attr_e( 'Remove', 'popular-ecommerce' ); ?></button>
    				</div>
    			</li>
    		</script>
    		<?php
        ...
        function get_icon_list(){
    	    $icons = include wp_normalize_path( get_template_directory() . '/themerelic/customizers/fontawesome.php' ); 
            if( $icons ){ ?>
