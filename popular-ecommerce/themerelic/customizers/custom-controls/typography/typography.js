 ...
 */
wp.customize.controlConstructor['relic-typography'] = wp.customize.Control.extend( {
    ready: function() {
		...
		renderSubControl = function( fontFamily, sub, startValue ) {
			...
			_.each( RaraAllFonts, function( font, i ) {
			...
			if ( true === isStandard ) {
				...
				if ( 2 > subList.length ) {
					...
					control.container.find( '.relic-variant-wrapper' ).css( 'display', 'none' );
				} else {
					control.container.find( '.relic-variant-wrapper' ).css( 'display', 'block' );
				}
